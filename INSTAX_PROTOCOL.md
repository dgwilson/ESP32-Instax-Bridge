...
**Model Number Values:**
- Link 3: `"FI033"`
- Link 2: `"FI032"` (unconfirmed)
- Link 1: `"FI031"` (unconfirmed)
...
**Info Service (Battery/Film Data):**
- Service UUID: `0000D0FF-3C17-D293-8E48-14FE2E4DA212`
- Data Characteristic: `0000FFD2-3C17-D293-8E48-14FE2E4DA212` (Read/Notify)

> **Note:** Link 3 uses the standard print service for actual printing operations. The Link 3-specific services appear to be for status monitoring and device information only.
...
| `BATTERY_INFO` | `0x01` | `[State, Percentage]` | Battery state and charge percentage (0-100) |
| `PRINTER_FUNCTION_INFO` | `0x02` | `[FunctionByte]` | Photos remaining (bits 0-3), charging flag (bit 7) |
...
- Accelerometer data format (3x int16 + 1x uint8)
- Tilt-to-zoom user interaction pattern
- Shutter button user interaction pattern

**❓ Unknown / Needs Investigation:**
- Shutter button BLE notification mechanism
- Exact accelerometer value ranges for each axis
...
**🔬 Future Work:**
1. Capture BLE packets during shutter button press to identify notification characteristic
2. Map accelerometer value ranges to real-world angles (degrees of tilt)
3. Determine orientation byte state meanings (portrait/landscape/upside-down)
4. Investigate AXIS_ACTION_SETTINGS configuration options
5. Implement simulator support for virtual accelerometer values and shutter button trigger
...
**Status:** PARTIAL COMPATIBILITY - ESP32 simulator can be discovered and connected by official Fujifilm Instax apps, but UI integration is incomplete.
...
The ESP32 Instax Bridge simulator (`/Users/dgwilson/Projects/ESP32-Instax-Bridge`) has achieved partial compatibility with the official Fujifilm Instax Square Link app:

✅ **What Works:**
- BLE advertising matches real Square Link printer exactly
- Official app discovers simulator in device list
- Connection establishes and remains stable
- All protocol queries answered correctly (battery, film count, print history)
- Protocol handshake completes successfully

❌ **What Doesn't Work:**
- Official app UI shows "Device not connected" despite active connection
- Film count and battery status display as "???" instead of actual values
- Cannot initiate printing from official app

### Key Findings

**Critical BLE Advertising Requirements:**
- Manufacturer Data must match exactly: `D8 04 05 00` (Company ID 0x04D8 = Fujifilm)
- TX Power must be advertised: 3 dBm
- Device name pattern: `INSTAX-XXXXXXXX` (8 digits)

**Device Information Service:**
- Model Number: Must be user-facing name ("SQ20") not internal code ("FI020")
- Firmware/Software Revision: Must include period ("01.01" not "0101")

**Protocol Command Differences:**
The official Instax app uses **different operation codes** than Moments Print app:
...
The ESP32 simulator now supports **both** protocol variants simultaneously.
...
- Multiple protocol command variants exist for the same queries
- Connection stability ≠ UI recognition (protocol can be 100% compatible but UI may still show "not connected")
- Model number validation occurs both in advertising and Device Information Service

### Future Investigation

To achieve full official app compatibility:
1. **Bluetooth packet sniffing** of real device during active printing (not just idle connection)
2. **Response format experimentation** to match real device byte-for-byte
3. **UI behavior testing** to determine if status shows during printing vs idle screen
4. **Firmware version validation** to check if specific versions are whitelisted
